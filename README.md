...
<a href="#-design-system"><img src="https://img.shields.io/badge/🎨-Design%20System-pink?style=for-the-badge" /></a>
...
<td align="center" width="50%">
<img src="https://organic-jade-ogvf7vxp.edgeone.dev/WhatsApp%20Image%202026-07-24%20at%2014.49.36.jpeg" width="100%"/>
...
```mermaid
flowchart LR
    A[Image Collection] --> B[Manual Annotation]
...
flowchart TD
    ...
    B --> C[Dashboard]
    C --> D[Upload Component]
    D --> E[AI Processing]
    E --> F[Confidence Evaluation]
    F -->|Low Confidence| G[Human Review]
    F -->|High Confidence| H[Interactive 3D Inspection]
    G --> H
    H --> I[Component Intelligence]
    I --> J[Decision Engine]
    J --> K[Generate Report]
    K --> L[Start New Inspection]
    L --> D
```
...
flowchart LR
    U[User] --> FE[Frontend]
    FE --> API[Backend API]
    API --> GEM[Gemini Vision API]
    GEM --> RES[AI Response]
    RES --> PROC[Processing Engine]
    PROC --> CONF[Confidence Evaluation]
    CONF --> VIEW[Inspection Viewer]
    VIEW --> DEC[Decision Engine]
    DEC --> REP[Report Generator]
    REP --> DL[User Download]
```
...
```mermaid
flowchart LR
    A[Click Component] --> B[Camera Zoom]
    B --> C[Component Highlight]
    C --> D[Rotation Enabled]
    D --> E[AI Info Panel Opens]
    E --> F[Defect Highlight Displayed]
    F --> G[Inspection Details Loaded]
```
...
```mermaid
flowchart LR
    A[High Confidence ≥90%] --> B[Automatic Decision]
    C[Medium Confidence 60-89%] --> D[Engineer Validation Recommended]
    E[Low Confidence <60%] --> F[Mandatory Human Review]
```
...
<tr><td>Manoj</td><td>AI / Backend Lead</td></tr>
<tr><td>Teammate 2</td><td>Frontend / 3D Viewer</td></tr>
<tr><td>Teammate 3</td><td>Design / Docs</td></tr>
</table>
